...
lower_lane = np.uint8([180, 18, 255])
upper_lane = np.uint8([0, 0, 231])
kernel_lane = 5
canny_low_lane = 75
canny_high_lane = 250
hough_rho_lane = 1
hough_theta_lane = np.pi / 180
hough_threshold_lane = 50
hough_max_line_gap_lane = 50
...
while cap.isOpened():
    ...
    # Lane detection
    hsv_lane = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    mask_lane = cv2.inRange(hsv_lane, lower_lane, upper_lane)
    edges_lane = cv2.Canny(mask_lane, canny_low_lane, canny_high_lane)
    
    lines_lane = cv2.HoughLinesP(edges_lane, hough_rho_lane, hough_theta_lane, hough_threshold_lane, maxLineGap=hough_max_line_gap_lane)

    if lines_lane is not None:
        for i in range(0, len(lines_lane)):
            l = lines_lane[i][0]
